...
If the user wants raw trending volume rather than a curated read, prefer `social-context-trending`. If they want a time-series view, use `social-context-pulse`.
...
3. Run via Bash:
   ```bash
   npx social-context@latest digest "<TOPIC>" --since <WINDOW> --format md
   ```
...
> → Run: `npx social-context@latest digest "Claude 4.5" --since 24h --format md`
